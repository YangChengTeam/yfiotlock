 ...
 * 不定长加密,pkcs5padding
 */
char *AES_128_ECB_PKCS5Padding_Encrypt(const char *in, const uint8_t *skey) {
    int BUFFER_SIZE = sizeof(skey) + 25;
    uint8_t key[BUFFER_SIZE];
    strcpy(key, skey);
    strcat(key, "28aed2a6abf7158809cf4f3c");
    key[BUFFER_SIZE - 1] = '\0';
 ...
 * 不定长解密,pkcs5padding
 */
char *AES_128_ECB_PKCS5Padding_Decrypt(const char *in, const uint8_t *skey) {
    int BUFFER_SIZE = sizeof(skey) + 25;
    uint8_t key[BUFFER_SIZE];
    strcpy(key, skey);
    strcat(key, "28aed2a6abf7158809cf4f3c");
